#!/usr/bin/env python3
"""
Hermes Agent Universal - Cross-Platform Build Script
...
def build_windows():
    ...
    icon_path = str(INSTALLER_DIR / "hermes.ico") if (INSTALLER_DIR / "hermes.ico").exists() else None
    ...
    print(f"Windows build complete: {DIST_DIR / 'hermes-agent.exe'}")
...
OutFile "{DIST_DIR / 'HermesAgent-Setup.exe'}"
InstallDir "$PROGRAMFILES\\HermesAgent"
...
Section "Install"
  ...
  CreateShortCut "$DESKTOP\\Hermes Agent.lnk" "$INSTDIR\\hermes-agent.exe"
  CreateDirectory "$SMPROGRAMS\\Hermes Agent"
  CreateShortCut "$SMPROGRAMS\\Hermes Agent\\Hermes Agent.lnk" "$INSTDIR\\hermes-agent.exe"
...
Section "Uninstall"
  ...
  RMDir /r "$SMPROGRAMS\\Hermes Agent"
SectionEnd
"""
    ...
    try:
        ...
        print(f"Windows installer created: {DIST_DIR / 'HermesAgent-Setup.exe'}")
...
def create_linux_appimage():
    ...
    appdir = BUILD_DIR / "HermesAgent.AppDir"
    if appdir.exists():
    ...
    # Copy binary
    binary_src = DIST_DIR / "hermes-agent"
    if binary_src.exists():
        shutil.copy2(binary_src, appdir / "usr" / "bin" / "hermes-agent")
...
"""
    (appdir / "hermes-agent.desktop").write_text(desktop_entry, encoding="utf-8")
    (appdir / "usr" / "share" / "applications").mkdir(parents=True, exist_ok=True)
    shutil.copy2(appdir / "hermes-agent.desktop", appdir / "usr" / "share" / "applications" / "hermes-agent.desktop")

    print(f"AppImage structure created at: {appdir}")
    print("Run `appimagetool` to create the final AppImage:")
    print(f"  appimagetool {appdir} {DIST_DIR / 'HermesAgent-x86_64.AppImage'}")
...
# 在 Termux 中运行此脚本以安装 Hermes Agent

echo "=== Hermes Agent Universal - Android Install ==="
echo ""
...
# 安装Hermes Agent
echo "[4/5] 安装Hermes Agent Universal..."
...
echo "  hermes desktop             # 桌面Web UI (需要安装Termux:X11)"
echo ""
echo "配置API Key:"
